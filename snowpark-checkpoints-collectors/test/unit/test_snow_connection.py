...
def test_create_snowflake_table_from_parquet():
    ...
    output_directory_path_full = os.path.abspath(output_directory_path)
    ...
    assert mocked_session.method_calls[1] == call.sql(
        f"PUT '{Path(expected_file.as_posix()).as_uri()}' "
...
def test_create_snowflake_table_from_parquet_tmp_folder():
    mocked_session = MagicMock()
    mock_df = MagicMock()
    mock_read = MagicMock()
    mock_save_as_table = MagicMock()
    ...
    output_path = os.path.join(temp_dir, "test_create_snowflake_table_from_parquet_tmp")
