...
def test_performance_mode_schema(telemetry, singleton):
    """
    Test the performance of the schema exucution mode.
    ...
    - limits_mode_dataframe: A dictionary containing the memory and time limits in schema execution mode  for the test.
    ...
    with tempfile.TemporaryDirectory(dir=(os.path.join(getcwd(),"tests"))) as temp_dir:
        ...
        performance_test(PACKAGE_NAME_COLLECTORS,input_name[PACKAGE_NAME_COLLECTORS],SIZE,EXECUTION_MODE, limits_mode_schema, temp_path)
        performance_test(PACKAGE_NAME_VALIDATORS,input_name[PACKAGE_NAME_VALIDATORS],SIZE,EXECUTION_MODE, limits_mode_schema, temp_path)

if __name__ == "__main__":
    test_performance_mode_schema() 