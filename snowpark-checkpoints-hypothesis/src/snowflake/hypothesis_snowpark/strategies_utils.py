...
def generate_snowpark_dataframe(
    ...
    with temporary_random_seed():
        LOGGER.info(
            "Creating Snowpark DataFrame with schema=%s and size=%s",
            snowpark_schema,
            len(data),
        )
...
def apply_custom_null_values(
    ...
    for column, target_null_proportion in null_proportions.items():
        if column not in df_copy.columns:
            LOGGER.warning(
                "Column '%s' not found in DataFrame during null value correction. Skipping it.",
                column,
            )
            continue
...
def replace_surrogate_chars(df: pd.DataFrame, columns: list[str]) -> pd.DataFrame:
    ...
    for column in columns:
        if column not in df_copy.columns:
            LOGGER.warning(
                "Column '%s' not found in DataFrame during surrogate replacement. Skipping it.",
                column,
            )
            continue

        LOGGER.info("Replacing surrogate characters in column '%s'", column)
